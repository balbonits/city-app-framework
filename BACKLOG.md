...
- **`decision-patterns/codebase-shape.md`.** Reference doc summarizing how five FAANG/MAANG companies organize their codebases, framed around two orthogonal axes (repo: mono vs. poly; deploy unit: single vs. many) rather than the conventional "monolith vs. microservices" framing. Per-claim inline citations to primary sources, supplemented with named ex-employee accounts (Hacker News, journalism) where official documentation is sparse — particularly Apple. Recurring-patterns table grades each pattern (trunk-based dev, feature flags, continuous deploy, code review, "you build it, you run it", tooling investment) by per-company evidence quality, with explicit gaps where no source was located. Surfaced from the "Postgres replaces microservices" video discussion and the follow-up "what do FAANG actually do?" deep-dive; six sub-agents dispatched across two rounds of research. ([`f6102e6`](https://github.com/balbonits/city-app-framework/commit/f6102e6))
